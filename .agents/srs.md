...
- **Çoklu Ürün Seçimi ve Toplu Silme (Bulk Delete):**
  - Kullanıcılar envanter listesindeki ürünleri tek tek seçebilir, arama sonucundaki tüm ürünleri "Tümünü Seç" butonu veya sütun başlığındaki master checkbox ile seçebilirler.
  - Seçim yapıldığında ekranda dinamik olarak "Seçimleri Sil" barı görüntülenir. Toplu silme işlemi onay modalı (`Seçili Ürünleri Sil`) onaylandıktan sonra Firestore `writeBatch` kullanılarak tek seferde atomik olarak gerçekleştirilir.
