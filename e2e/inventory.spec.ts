...
test.describe('Inventory Flow', () => {
  ...
  test('should perform full product CRUD cycle', async ({ page }) => {
    ...
    await page.locator('input[name="salePrice"]').fill('25.50');
    ...
    // 3. Edit Product
    const productRow = page.getByRole('row', { name: new RegExp(uniqueName) });
    await productRow.getByLabel('Düzenle').click();
    await expect(
      page.getByRole('heading', { name: 'Ürün Düzenle' })
    ).toBeVisible();
    await page.locator('input[name="salePrice"]').fill('30.00');
    ...
    await page
      .locator('input[placeholder="Ürün adı veya barkod ile ara..."]')
      .fill(uniqueName);
    await expect(productRow.getByText('₺30.00')).toBeVisible();

    // 5. Delete Product
    await productRow.getByLabel('Sil').click();
